...
def validate_data(qcplatform, current_data_df: pd.DataFrame, additional_data_df: pd.DataFrame, mode: int) -> bool:
    is_valid = True
    if additional_data_df is not None and len(additional_data_df) > 1:
        ...
        is_valid = validate_data_for_time_gaps(df, fuzzy_seconds=qcplatform.accept_time_difference)
    return is_valid


def validate_data_for_spike(qcplatform, current_data_df: pd.DataFrame, additional_data_df: pd.DataFrame,
...
def validate_additional_data(qcplatform, qcinput: QCInput_df):
    ...
    is_valid = True
    if not validate_data(qcplatform, qcinput.current_data, qcinput.historical_data, 1):
        is_valid = False
        qcinput.historical_data = pd.DataFrame.from_dict({})
        logging.debug("Removing historical data due to time gaps")

    if not validate_data(qcplatform, qcinput.current_data, qcinput.future_data, 2):
        is_valid = False
        qcinput.future_data = pd.DataFrame.from_dict({})
        logging.debug("Removing future data due to time gaps")

    if not validate_data_for_spike(qcplatform, qcinput.current_data, qcinput.historical_data, qcinput.future_data):
        qcinput.future_data = pd.DataFrame.from_dict({})
        qcinput.historical_data = pd.DataFrame.from_dict({})
